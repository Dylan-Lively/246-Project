...
Run:
    ...
    python hand_tracker_pi.py --no-serial --no-display   # headless, fastest
"""
...
SERIAL_BAUD     = 115200
SEND_HZ         = 20
# Lower resolution = dramatically faster landmark detection
CAM_W           = 640
CAM_H           = 480
SMOOTH_ALPHA    = 0.25
DEADBAND        = 0.005
...
# How many camera frames to skip between gesture recognition runs.
# 0 = every frame, 2 = every 3rd frame, etc.
# Landmarks still run every frame; gesture classification is the slow part.
...
class SerialWriter:
    """
    Pushes packets to a background thread so serial.write() never stalls
    the main loop. Drops stale packets if the queue backs up.
    """
    def __init__(self, ser):
        self._ser   = ser
        self._queue = deque(maxlen=4)   # drop old data, not new
...
def main():
    ...
    parser.add_argument("--no-display", action="store_true",
                        help="Skip imshow entirely (headless / SSH mode)")
    ...
    if not args.no_serial:
        ...
        if args.port is None:
            ...
            return
        _ser   = serial.Serial(args.port, SERIAL_BAUD, timeout=1)
        time.sleep(2)
        writer = SerialWriter(_ser)
    ...
    # Use LIVE mode + callback — non-blocking, returns immediately
    # and fires on a MediaPipe-internal thread.
    latest_result     = [None]
    ...
    options = GestureRecognizerOptions(
        ...
        running_mode=VisionRunningMode.LIVE_STREAM,   # ← non-blocking
        num_hands=1,
    ...
    with GestureRecognizer.create_from_options(options) as recognizer:
        while True:
            ...
            # Send every frame to MediaPipe LIVE_STREAM — it processes
            # asynchronously and fires on_result when done.  Timestamps
            # must be strictly increasing (milliseconds).
            recognizer.recognize_async(mp_img, int(now * 1000))

            # Grab latest result (may be a frame or two behind — that's fine)
            with result_lock:
            ...
            if not args.no_display:
                ...
                if frame_idx % 60 == 0:
                    print(f"FPS: {fps:.1f}")
                # Allow clean exit via KeyboardInterrupt
                time.sleep(0)
